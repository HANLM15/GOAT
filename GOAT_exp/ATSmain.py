...
import sys
sys.path.append('../')
from dataloader import train_valid_test_iemocap_dataloader
from torch import nn, optim
import os
from pathlib import Path
import ATGO.ATSutils
import datetime
from ATGO.ATSmodel import TextEncoder,AudioEncoder,SharedHead
import logging
import numpy as np
...
def main():
    ...
    model_file_path = '/mnt/cxh10/database/hanlm/SSLER/ATGO/ATSmodel.py'
    with open(model_file_path, 'r') as file:
        file_contents = file.read()
        logger.info(file_contents)
    logger.info(cfg)

    # alpha = cfg["train"]["optimizer"]["lr"]
    for fold in sessions:
        ...
        # 动态平衡不同模态的学习率
        text_lr = cfg["train"]["optimizer"]["lr"] * 2.0  # 可根据需求调整
        text_optimizer = optim.RMSprop(list(text_encoder.parameters()) + list(shared_head.parameters()), lr=text_lr)
        audio_lr = cfg["train"]["optimizer"]["lr"] * 1.0  # 可根据需求调整
        audio_optimizer = optim.RMSprop(list(audio_encoder.parameters()) + list(shared_head.parameters()), lr=audio_lr)
        text_scheduler = optim.lr_scheduler.CosineAnnealingLR(text_optimizer, T_max=cfg["train"]["epoch"])
        audio_scheduler = optim.lr_scheduler.CosineAnnealingLR(audio_optimizer, T_max=cfg["train"]["epoch"])
        test_wa, test_ua, test_wf1 = ATGO.ATSutils.train_one_fold(logger, save_dir, fold, [audio_encoder, text_encoder, shared_head],
                                                                   train_loader, val_loader, test_loader,
                                                                   [audio_optimizer, text_optimizer],
                                                                   [audio_scheduler, text_scheduler],
                                                                   device, cfg, continue_training=False)
